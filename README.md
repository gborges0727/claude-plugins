...
| `strip-attribution.py` | `PreToolUse` hook | Removes AI-attribution footers from GitHub writes. Enforces the `CONVENTIONS.md` ban mechanically |
...
`CONVENTIONS.md` bans AI-attribution trailers on commit messages and PR bodies, and says the ban holds even when a tool guide asks for one. Several harnesses ask for one anyway. Claude Code on the web instructs the model to append a `Generated by [Claude Code]` footer to every GitHub comment and PR body it writes.

That conflict is instructions against instructions, and the harness copy sits in the system prompt, which outranks a file injected by a `SessionStart` hook. Wording the convention more forcefully does not change the ordering. The `PreToolUse` hook removes the question from the model's hands by editing the call itself.
