...
That conflict is instructions against instructions, both sitting in the system prompt, and the model can resolve it either way on any given call. Two things take the question out of the model's hands. A settings key stops the instruction from reaching the prompt, and the `PreToolUse` hook edits any call that carries a footer anyway.

The settings key is `attribution` in `~/.claude/settings.json`. Claude Code reads it when it builds the prompt. Unset, the prompt asks for a `Co-Authored-By` trailer on every commit, a `Generated with Claude Code` line on every PR body, and a claude.ai session link on both in cloud sessions. `scripts/setup.sh --attribution off`, the default, writes empty text for the commit and the PR and turns the link off.

`--attribution on` deletes the key for anyone who wants the default back. The cloud bootstrap writes the same key into the VM's user settings. This is the only fix that covers a commit or a `gh pr create` run through Bash, since neither hook below watches that tool.
